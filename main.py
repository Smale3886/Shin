...
import os  # To get the PORT environment variable for Render
from flask import Flask  # The Flask web server
...
# --- FLASK WEB SERVER (for Render/Uptime Robot) ---
flask_app = Flask(__name__)

@flask_app.route('/')
def index():
    """A simple route to check if the bot is alive (for Uptime Robot)."""
...
def main() -> Application:
    """Configures and sets up the bot application."""
...
if __name__ == "__main__":
    ...
    # 4. Get the port from the environment variable (for Render)
    # Default to 8080 for local testing if PORT isn't set
    port = int(os.environ.get('PORT', 8080))

    # 5. Start the Flask web server on the main thread
    # host='0.0.0.0' is crucial for it to be reachable by Render
